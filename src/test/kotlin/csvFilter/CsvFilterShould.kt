...
class CsvFilterShould {
    ...
    @Test
    fun lines_with_net_fields_correct_are_not_filtered(){
        val result = filter.apply(fileWithOneInvoiceLineHaving(ivaTax = "20", igicTax = emptyField))
    ...
    fun there_must_be_one_header_for_the_invoice(){
        ...
        val result = CsvFilter().apply(listOf("", invoiceLine))

        assertThat(result).isEqualTo(listOf(""))
    }

    @Test
    fun tax_identification_fields_are_mutually_exclusive(){
        val result = filter.apply(fileWithOneInvoiceLineHaving(cif = "B76430134", nif = "50584422Q"))

        assertThat(result).isEqualTo(emptyDataFile)
    }

    @Test
    fun there_must_be_at_least_one_tax_identification_for_the_invoice(){
        val result = filter.apply(fileWithOneInvoiceLineHaving(cif = emptyField, nif = emptyField))
    ...
    fun lines_with_tax_identification_fields_correct_are_not_filtered(){
        val result = filter.apply(fileWithOneInvoiceLineHaving(cif = "976430134", nif = emptyField))

        assertThat(result).isEqualTo(emptyDataFile)
    }

    @Test
    fun lines_repeated_are_filtered(){
        val invoiceLine = "1,02/05/2019,1000,810,19,,ACER Laptop,B76430134,"

        val result = CsvFilter().apply(listOf(headerLine, invoiceLine, invoiceLine))

        assertThat(result).isEqualTo(listOf(headerLine))
    }

    private fun fileWithOneInvoiceLineHaving(ivaTax: String = "19", igicTax: String = emptyField, concept: String = "irrelevant", cif: String = "B76430134", nif: String = emptyField ) : List<String> {
        val invoiceId = "1"
